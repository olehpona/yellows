...
public class RunContextTest {
    public static class TestSubGraphBuilder {
        ...
        public SubGraph build() {
            ...
            return new SubGraph(inDegree, childrenStart, childrenFlat, localToGlobal, 0);
        }
    }
    ...
    void mergeOutput_decrementInDegree_returnReadyNodes() {
        ...
        NodeData aData = new NodeData("",0,List.of(),
                List.of(
                        new CompiledNode.PathPair(globalPath, remotePath)
                )
        );

        NodeData bData = new NodeData("",0,List.of(
                new CompiledNode.PathPair(globalPath, remotePath)
        ), List.of());
        ...
        when(outputValue.resolvePath(anyIterable(), eq(dict))).thenReturn(MissingValue.INSTANCE);
    ...
    void mergeOutput_nodeCancelledThenWanted_returnReadyNodes() {
        ...
        NodeData data = new NodeData("",0,List.of(),List.of());
    ...
    void buildInputContext_correctOrder_returnCorrectContext() {
        ...
        when(root.resolvePath(anyIterable(), eq(dict))).thenReturn(MissingValue.INSTANCE);
    ...
    void buildInputContext_incorrectOrder_throwNodeNotReadyErr() {
        ...
        NodeData data = new NodeData("",0,List.of(
                new CompiledNode.PathPair(globalPath, remotePath)
        ), List.of());
